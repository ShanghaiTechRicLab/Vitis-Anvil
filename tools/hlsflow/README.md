...
`hlsflow` parses Vitis HLS csynth / cosim reports, renders rich console output,
...
| `kernel`, `platform`, `target` | strings (target ∈ {csynth, cosim}) |
| `git_commit`, `build_dir`, `vitis_version`, `status`, `timestamp` | strings |
| `reports` | dict (paths to source XML / HTML / TXT) |
| `metrics` | dict (worst_loop_ii, latency_min/max, lut, ff, dsp, bram_18k, uram, target_clock_ns, estimated_clock_ns, timing_met, …) |
